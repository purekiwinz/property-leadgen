...
function renderSaleTile(sale: Sale | null): string {
  if (!sale) {
    return `<div class="sale-tile"></div>`;
  }
...
function renderFrontAd(suburb: Suburb): string {
  const qr = QR_MAP[suburb];
  return `<div class="ad">
    <div class="col-agent">
      <img class="agent-img" src="/agent_transparent.webp" alt="Ed Scanlan">
    </div>
    <div class="col-content">
      <div class="top-group">
        <div class="logo-area">
          <svg aria-label="Professionals Hibiscus Coast" style="width:150px;height:38px;display:block"><use href="#prof-logo"/></svg>
        </div>
        <div class="red-rule"></div>
        <p class="quote">Free Market Appraisal<span class="quote-body">No Pressure. Just Real, Local Insight.</span></p>
      </div>
...
function renderBackAd(suburb: Suburb, sales: Sale[]): string {
  const tiles = [0, 1, 2].map(i => renderSaleTile(sales[i] ?? null)).join('<div class="tile-sep"></div>');
  return `<div class="ad back-ad">
...
function generateHtml(sales: Sale[]): string {
  ...
  const qrScript = SUBURBS.map(suburb => {
    const qr = QR_MAP[suburb];
    return `makeQR('${qr.id}', '${qr.url}');`;
  }).join('\n  ');

  const sections = SUBURBS.map(suburb => {
    const suburbSales = getSalesForSuburb(sorted, suburb);
    return `
  <div class="suburb-section">
    <div class="suburb-header">
      <span class="suburb-sect-label">${esc(suburb)}</span>
      <span class="suburb-sect-sub">Front &middot; Back</span>
    </div>
    <div class="flyer-pair">
      <div class="flyer-col">
        <span class="side-label">Front</span>
        ${renderFrontAd(suburb)}
      </div>
      <div class="flyer-col">
        <span class="side-label">Back</span>
        ${renderBackAd(suburb, suburbSales)}
      </div>
    </div>
  </div>`;
  }).join('');
...
<head>
  ...
  <title>Professionals Hibiscus Coast — DLE Flyers (Two-Sided)</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Poppins:ital,wght@0,300;0,400;0,600;0,700;0,800;0,900;1,300;1,400&family=Source+Serif+4:ital,opsz,wght@1,8,400;1,8,700&display=swap" rel="stylesheet">
  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .page-header h1 { font-size: 13px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #FF4753; margin-bottom: 4px; }
    .page-header p { font-size: 12px; color: #555; letter-spacing: 0.04em; }

    /* ── suburb section ── */
    .suburb-section { display: flex; flex-direction: column; gap: 8px; }
    .suburb-header { display: flex; align-items: baseline; gap: 8px; }
    .suburb-sect-label { font-size: 11px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #FF4753; }
    .suburb-sect-sub { font-size: 10px; color: #444; letter-spacing: 0.06em; }
    .flyer-pair { display: flex; gap: 28px; align-items: flex-start; }
    .flyer-col { display: flex; flex-direction: column; gap: 6px; }
    .side-label { font-size: 9px; font-weight: 600; letter-spacing: 0.14em; text-transform: uppercase; color: #444; }

    /* ══════════════════════════════════════
       DLE AD — 210mm × 99mm (landscape)
       Display at 560px × 264px
    ═══════════════════════════════════════ */
    .ad {
      width: 560px;
      height: 264px;
    ...
    /* ── FRONT: agent col ── */
    .col-agent { width: 165px; flex-shrink: 0; overflow: hidden; position: relative; background: #373D40; }
    .agent-img { width: 100%; height: 100%; object-fit: cover; object-position: 60% top; display: block; }

    /* ── FRONT: content col ── */
    .col-content { flex: 1; display: flex; flex-direction: column; justify-content: space-between; padding: 16px 14px 16px 18px; overflow: hidden; }
    .top-group { display: flex; flex-direction: column; }
    .logo-area { margin-bottom: 8px; flex-shrink: 0; }
    .red-rule { width: 100%; height: 2px; background: #FF4753; margin-bottom: 8px; flex-shrink: 0; }
    .quote { font-size: 11px; font-weight: 700; color: #FF4753; font-family: 'Source Serif 4', Georgia, serif; font-style: italic; line-height: 1.3; }
    .quote-body { font-family: 'Poppins', sans-serif; font-style: normal; font-weight: 400; font-size: 9.5px; color: #fff; display: block; margin-top: 3px; line-height: 1.5; }
    .headline-block { display: flex; flex-direction: column; }
    .check-your { font-size: 10px; font-weight: 400; color: #fff; letter-spacing: 0.04em; line-height: 1.4; }
    .suburb-name { font-family: 'Source Serif 4', Georgia, serif; font-size: 36px; font-weight: 700; font-style: italic; color: #FF4753; letter-spacing: -0.01em; line-height: 1; white-space: nowrap; }
    .selling-price { font-size: 10px; font-weight: 700; color: #fff; line-height: 1.35; letter-spacing: 0.01em; text-transform: uppercase; margin-top: 3px; }

    /* ── FRONT: QR col ── */
    .col-qr { width: 148px; flex-shrink: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 7px; padding: 14px; background: rgba(0,0,0,0.22); border-left: 1px solid rgba(255,255,255,0.06); }
    .qr-wrap { background: #fff; border-radius: 6px; padding: 6px; display: inline-flex; flex-shrink: 0; }
    .qr-wrap svg, .qr-wrap img { display: block; width: 76px; height: 76px; }
    .qr-cta { font-size: 9px; font-weight: 700; color: #FF4753; font-family: 'Source Serif 4', Georgia, serif; font-style: italic; text-align: center; line-height: 1.4; }
    .contact-block { width: 100%; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 7px; display: flex; flex-direction: column; align-items: center; gap: 1px; }
    .contact-name { font-size: 10px; font-weight: 700; color: #fff; text-align: center; }
    .contact-phone { font-size: 10px; font-weight: 600; color: #fff; text-align: center; }
    .contact-license { font-size: 6px; font-weight: 400; color: #fff; text-align: center; line-height: 1.4; margin-top: 1px; white-space: nowrap; }

    /* ── BACK: layout ── */
    .back-ad { flex-direction: column; }
    ...
    .bh-sep { width: 1px; height: 26px; background: rgba(255,255,255,0.15); flex-shrink: 0; }
    .bh-heading { display: flex; flex-direction: column; gap: 1px; }
    .bh-suburb { font-family: 'Source Serif 4', Georgia, serif; font-style: italic; font-weight: 700; font-size: 17px; color: #FF4753; line-height: 1; }
    .bh-title { font-size: 8.5px; font-weight: 600; color: #fff; letter-spacing: 0.1em; text-transform: uppercase; }
    .bh-spacer { flex: 1; }
    .bh-tagline { font-size: 9px; color: #fff; text-align: right; line-height: 1; font-weight: 400; white-space: nowrap; }

    .back-red-rule { width: 100%; height: 2px; background: #FF4753; flex-shrink: 0; }

    /* ── BACK: sales row ── */
    .sales-row { display: flex; flex: 1; min-height: 0; padding: 6px; gap: 5px; }

    .sale-tile { flex: 1; background: #373D40; border-radius: 0; overflow: hidden; display: flex; flex-direction: column; min-width: 0; }
    .tile-sep { width: 1px; background: rgba(255,255,255,0.25); align-self: stretch; flex-shrink: 0; }
    ...
    .tile-addr { font-size: 10px; font-weight: 700; color: #fff; line-height: 1.3; }
    .tile-sub { font-family: 'Source Serif 4', Georgia, serif; font-style: italic; font-weight: 400; color: #FF4753; }
    .tile-feats { display: flex; align-items: center; gap: 3px; font-size: 9px; font-weight: 600; color: #fff; flex-wrap: nowrap; }
    .feat { display: flex; align-items: center; }
    .feat-sep { color: #FF4753; font-weight: 300; font-size: 11px; line-height: 1; }

    /* instructions */
    .instructions { background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 8px; padding: 24px 36px; max-width: 800px; text-align: center; font-size: 12px; color: #666; line-height: 1.8; }
    .instructions h3 { font-size: 12px; font-weight: 600; color: #aaa; letter-spacing: 0.08em; text-transform: uppercase; margin-bottom: 10px; }
    .instructions strong { color: #999; }
    .instructions code { background: #222; color: #FF4753; padding: 1px 6px; border-radius: 3px; font-size: 11px; }
    ...
    @media print {
      ...
      .flyer-col { gap: 0; }
      .ad { width: 210mm; height: 99mm; border-radius: 0; box-shadow: none; page-break-inside: avoid; page-break-after: always; }
      .col-agent { width: 29%; }
      .col-qr { width: 26%; }
    }
  </style>
...
<div class="page-header">
  <h1>Professionals Hibiscus Coast — DLE Mailbox Drop (Two-Sided)</h1>
  <p>Ed Scanlan &nbsp;&middot;&nbsp; 210 &times; 99 mm landscape &nbsp;&middot;&nbsp; Front + Back per suburb &nbsp;&middot;&nbsp; Sales loaded live from database</p>
</div>
...
<div class="instructions">
  ...
  Each DLE is <strong>210 &times; 99 mm</strong> landscape (front shown left, back shown right).<br>
  To export: <strong>File &rarr; Print &rarr; Save as PDF</strong>, landscape orientation, no scaling.<br>
  Send fronts and backs as separate files to your print house, or screenshot each at 2&times; for a <strong>1120 &times; 528 px</strong> digital version.<br><br>
  QR codes link to suburb-specific landing pages tagged <code>utm_medium=print</code> so<br>
  mailbox drop leads appear separately in HubSpot.
</div>
